...
export default abstract class StickyObject<P extends StickyObjectProps> extends ComponentCompat<P> {
    ...
    protected stickyTypeMultiplier = 1;
    protected stickyVisiblity = false;
    protected containerPosition: StyleProp<ViewStyle>;
    protected currentIndex = 0;
    protected currentStickyIndex = 0;
    protected visibleIndices: number[] = [];
    protected bounceScrolling = false;
    ...
    private _previousStickyIndex = 0;
    private _nextStickyIndex = 0;
    private _firstCompute = true;
    private _smallestVisibleIndex = 0;
    private _largestVisibleIndex = 0;
    private _offsetY = 0;
    ...
    public renderCompat(): JSX.Element | null {
        ...
        const containerStyle = [
            { transform: [{ translateY: this._stickyViewOffset }] },
            !this.props.renderContainer && [{ width: this._scrollableWidth }, this.containerPosition],
        ];

        const content = (
            <Animated.View style={[containerStyle, !this.props.renderContainer && { position: 'absolute' }]}>
    ...
    public onScroll(offsetY: number): void {
        ...
        if (this._previousStickyIndex !== undefined) {
            if (
                this._previousStickyIndex * this.stickyTypeMultiplier >=
                this.currentStickyIndex * this.stickyTypeMultiplier
            ) {
            ...
            if (this._previousHeight && this._currentYd && scrollY && scrollY < this._currentYd) {
                if (scrollY > this._currentYd - this._previousHeight) {
                    ...
                    const translate =
                        (scrollY - this._currentYd + this._previousHeight) * (-1 * this.stickyTypeMultiplier);
        ...
        if (this._nextStickyIndex !== undefined) {
            if (
                this._nextStickyIndex * this.stickyTypeMultiplier <=
                this.currentStickyIndex * this.stickyTypeMultiplier
            ) {
                throw new CustomError(RecyclerListViewExceptions.stickyIndicesArraySortError);
            }
    ...
    protected stickyViewVisible(_visible: boolean, shouldTriggerRender = true): void {
    ...
    private _computeLayouts(newStickyIndices?: number[]): void {
        ...
        if (stickyIndices) {
            ...
            if (this.currentStickyIndex !== undefined) {
                ...
                this._currentYd =
                    this._currentY && this._currentHeight
                        ? this.getCurrentYd(this._currentY, this._currentHeight)
                        : undefined;
            }
            if (this._previousStickyIndex !== undefined) {
            ...
            if (this._nextStickyIndex !== undefined) {
                ...
                this._nextYd =
                    this._nextY && this._nextHeight ? this.getNextYd(this._nextY, this._nextHeight) : undefined;
            }
        }
    }
