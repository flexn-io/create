...
public class TvRemoteHandlerModule extends ReactContextBaseJavaModule {
    ...
    private final boolean enableKeyDownEvents = false;
    private final int longPressEventKey = 999;
    ...
    private boolean longPressEventSent = false;

    public TvRemoteHandlerModule(ReactContext context) {
    ...
    public void handleKeyEvent(KeyEvent ev) {
        ReactContext context = mReactContext;
            ...
            if(longPressEventActive) {
                if (!longPressEventSent) {
                    ...
                    dispatchEvent(
                            ...
                            mLastFocusedViewId,
                            !isSelectEvent(eventKeyCode) ? longPressEventKey : KeyEvent.ACTION_DOWN,
                            context
                    );
    ...
    private boolean shouldDispatchEvent(int eventKeyCode, int eventKeyAction, long time) {
        return KEY_EVENTS_ACTIONS.containsKey(eventKeyCode) && (
                (eventKeyAction == KeyEvent.ACTION_UP) ||
                        (eventKeyAction == KeyEvent.ACTION_DOWN && !longPressEventActive && !isSelectEvent(eventKeyCode)) ||
    ...
    private void dispatchEvent(String eventType, int targetViewId, int eventKeyAction, ReactContext context) {
        ...
        if (eventKeyAction == KeyEvent.ACTION_DOWN) {
            keyAction = "down";
        } else if (eventKeyAction == longPressEventKey) {
            keyAction = "longPress";
        }

        event.putString("eventType", eventType);
        event.putString("eventKeyAction", keyAction);
        if (targetViewId != View.NO_ID) {
            event.putInt("tag", targetViewId);
            event.putInt("target", targetViewId);
        }
        emitNamedEvent("onTVRemoteKey", event, context);
    }
