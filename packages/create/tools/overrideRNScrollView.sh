...
RN_TVOS_DIR="${CREATE_DIR}/../../../react-native-tvos"
RN_SCROLLVIEW_DIR="${RN_DIR}/Libraries/Components/ScrollView"
RN_TVOS_SCROLLVIEW_DIR="${RN_TVOS_DIR}/Libraries/Components/ScrollView"
MIN_RN_VERSION="68"

if [ -e "${RN_TVOS_DIR}/package.json" ] ; then 
  PACKAGE_VERSION=$(cat "${RN_TVOS_DIR}"/package.json \
    | grep version \
    | head -1 \
    | awk -F: '{print $2 }' \
    | sed 's/[",]//g')

  if [ -e "$RN_TVOS_SCROLLVIEW_DIR" ] && [ "${PACKAGE_VERSION:3:2}" -ge "${MIN_RN_VERSION}" ] ; then
      sed -i '' "s|'RCTScrollView'|Platform.isTV \&\& Platform.OS === 'android' ? 'RCTScrollViewTV' : 'RCTScrollView'|" "${RN_TVOS_SCROLLVIEW_DIR}/ScrollViewNativeComponent.js"
