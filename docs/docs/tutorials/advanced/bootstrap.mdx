...
To avoid having to configure plugins, engines and various other settings we will start with `@flexn/create-template-starter`, however unlike in quick start we will delete all the src files, meaning we get a fresh start and start focusing on what matters - the multiplatform code.
